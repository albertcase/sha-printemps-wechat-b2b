...
class ApiController extends Controller
{
	...
	public function actionName()
	{
		...
		while(!feof($handle)){
			...
			$command = Yii::app()->db->createCommand($sql);
			$command->bindParam(':cardno',trim($lineAry[0]),PDO::PARAM_STR);
			$command->bindParam(':firstname',trim($lineAry[1]),PDO::PARAM_STR);
			$command->bindParam(':secondname',trim($lineAry[2]),PDO::PARAM_STR);
			$command->bindParam(':bak',trim($lineAry[3]),PDO::PARAM_STR);
			$command->execute();
			$ok++;
		}
		fclose($handle);
